 ...
 * for example). This will look for paragraphs that are made up _exclusively_ of
 * an `iframe` tag (no other content) and replace it with the provided `iframe`
 * HTML. A suggested way of handling this would be something like this:
 *
 * ```html
 * <p>
 ...
 *       width="560"
 *       height="315"
 *       src="https://www.youtube-nocookie.com/embed/Jii84wxBUSw"
 ...
 * since it will be replaced by the `iframe` when rendered. Note that the HTML
 * code itself is 1px and white to make it less obtrusive in the Google Doc.
 */
