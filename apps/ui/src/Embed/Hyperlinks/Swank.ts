 ...
 *
 * The model that works most effectively would be represented in simple HTML as:
 *
 * ```html
 * <p>
 *   <a href="VIDEO_URL">VIDEO_TITLE</a><br />
 ...
 * video. Since Google Doc doesn't let you turn images into links, the resulting
 * is as described above.
 *
 * This can only handle one Swank link per paragraph because of this.
 */
export function Swank() {
  Helper.onGoogleDocEmbed<HTMLParagraphElement>(
    `p:has(${swankLink})`,
    (paragraphs) => {
      paragraphs.forEach((p) => {
        const url = (document.querySelector(swankLink) as HTMLAnchorElement)
          .href;
        const id = url.replace(/.*\/watch\/(.*)\??/, '$1');
        p.replaceWith(
          Helper.iframe(`https://play.vidyard.com/${id}?autoplay=0`)
        );
