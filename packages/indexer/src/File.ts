...
class File {
  ...
  public async fetchAsHtmlIfPossible() {
    switch (this.mimeType) {
      case MimeTypes.Google.Doc:
      case MimeTypes.Google.Sheet:
      case MimeTypes.Google.Slides:
        return await this.fetchAsCompleteHtml();
      case MimeTypes.Google.Shortcut:
  ...
  public async cache({
    ...
    const bucket = Client.getStorage().bucket(bucketName);
    if (
      force ||
